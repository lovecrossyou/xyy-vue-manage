    ...
    <div class="search_container">
      <el-form :inline="true" :model="formInline" class="demo-form-inline">
        <el-form-item label="标题">
          <el-input v-model="formInline.user" placeholder="输入标题"></el-input>
        </el-form-item>
...
export default {
  ...
  methods: {
    ...
    deleteArticle(arr) {
      ...
      this.$confirm(
        `确定要${arr.length > 1 ? "批量删除文章" : "删除此文章"}吗？`,
        "系统提醒",
        {
    ...
    createButton(h, row, code, text) {
      ...
      if (code === "passed") {
        ...
        dis =
          user.user_type > 2 && user.id !== row.user_id
            ? true
            : this.grade.updateArticle;
      } else if (code === "delete") {
        dis =
          user.user_type > 2 && user.id !== row.user_id
            ? true
    ...
    getActiveContent(id) {
      ...
      utils.ajax.call(
        this,
        "/getArticleById",
        Object.assign({ id }, this.search_data),
        (obj, err) => {
          this.loading = !1;
          if (err) {
